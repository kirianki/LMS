...
class AIAgentViewSet(viewsets.ViewSet):
    ...
    def parse_valuation(self, request):
        ...
        if not organization or not organization.is_ai_enabled:
            return Response({"error": "AI features are not enabled for this organization."}, status=status.HTTP_403_FORBIDDEN)
