...
/** Prettify Django app_label for display */
...
/** Prettify Django permission name: "Can view loan" → "View Loan" */
...
/** Get action type from codename for badge coloring */
...
export default function EditRolePage() {
        ...
        const fetchData = async () => {
            try {
                ...
                setFormData({
                    ...
                    approval_limit: (roleData.approval_limit || 0).toString(),
                    permission_ids: roleData.permission_ids || [],
                });

                setAllPermissions(perms);
                setIsSystemRole(roleData.is_system_role || false);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        try {
            await api.patch(`/roles/${params.id}/`, {
                ...formData,
            ...
            alert(error.response?.data?.name?.[0] || 'Failed to update role');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    if (isLoading) {
        return (
            <div className="flex h-[400px] items-center justify-center">
                <div className="text-muted-foreground animate-pulse">Loading role...</div>
            </div>
        );
    }
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">
                            Configure permissions — {selectedCount} of {totalPerms} selected
                        </p>
                    </div>
                </div>
                <button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="flex items-center justify-between gap-4 flex-wrap">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className="px-3 py-1.5 text-xs font-bold text-emerald-400 hover:bg-emerald-500/10 rounded-lg transition-colors"
                                        >
                                            Select All
                                        </button>
                                        <button
                                        ...
                                        <div key={group}>
                                            ...
                                            <div className="flex items-center justify-between px-4 py-3 bg-muted/30 hover:bg-muted/50 transition-colors">
                                                ...
                                                <button
                                                    ...
                                                    className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-colors ${allGroupSelected
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                                                    {perms.map((p: any) => {
                                                        ...
                                                        return (
                                                            <label
                                                                key={p.id}
                                                                className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors border-b border-border/50 last:border-b-0 ${isSelected
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border">
                    <p className="text-sm text-muted-foreground">
                        {selectedCount} permission{selectedCount !== 1 ? 's' : ''} assigned to this role
                    </p>
                    <div className="flex items-center gap-3">
                        <button
                            type="button"
                            onClick={() => router.back()}
                            className="px-6 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:bg-muted transition-colors"
                        >
                            Cancel
                        </button>
                        <button
