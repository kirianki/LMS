...
This module provides integration with Safaricom M-Pesa APIs for:
- B2C (Business to Customer) disbursements
- C2B (Customer to Business) payment collections
...
class MpesaService:
    ...
    def __init__(self, tenant_settings):
        ...
        self.base_url = self.PRODUCTION_URL if tenant_settings.mpesa_environment == 'production' else self.SANDBOX_URL
    
    def _get_access_token(self):
        """Get OAuth access token from M-Pesa."""
        if not self.settings.mpesa_consumer_key or not self.settings.mpesa_consumer_secret:
            raise ValueError("M-Pesa credentials not configured for this tenant")
