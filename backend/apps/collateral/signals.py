...
def trigger_collateral_valuation_workflow(sender, instance, created, **kwargs):
    if created:
        ...
        if organization:
            if not organization.is_automation_enabled:
                logger.info(f"Automation disabled for organization {organization.company_name}. Skipping valuation workflow.")
                return
        ...
        if not organization or not organization.smtp_host:
            logger.warning("No Email configuration found in Organization. Skipping automation.")
            return
            
        # 2. Find a Valuer for this type (scoped to organization)
        valuer = Valuer.objects.filter(
            organization=organization,
            valuation_types__contains=[instance.collateral_type], 
            is_active=True
        ).first()
        
        if not valuer:
            logger.warning(f"No active valuer found for type {instance.collateral_type} in organization {organization.company_name}. Skipping automation.")
            return
        ...
        Regards,
        {organization.company_name} Management
        """
        send_tenant_email(organization, borrower_subject, borrower_message, [borrower.email])
        ...
        Please provide a valuation report for the following asset (Organization: {organization.company_name}):
        ...
        System Agent
        """
        send_tenant_email(organization, valuer_subject, valuer_message, [valuer.email])
